...
#define TOT_MEM_IN_BYTES (1500 * 1024)
#define K (512 * 1024 / 64)
	CSS<4, K> *css = NULL;
	...
	for(int datafileCnt = START_FILE_NO; datafileCnt <= END_FILE_NO; ++datafileCnt)
	{
		...
		css = new CSS<4, K>(TOT_MEM_IN_BYTES);

		int packet_cnt = (int)traces[datafileCnt - 1].size();
		for(int i = 0; i < packet_cnt; ++i)
		{
			css->insert((uint8_t*)(traces[datafileCnt - 1][i].key));
...
#define HEAVY_HITTER_THRESHOLD(total_packet) (total_packet * 1 / 1000)
		...
		{
			if((int)heavy_hitters[i].second < HEAVY_HITTER_THRESHOLD(packet_cnt))
				continue;
