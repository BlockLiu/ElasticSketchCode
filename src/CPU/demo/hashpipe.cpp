...
#define TOT_MEM_IN_BYTES (600 * 1024)
	...
	for(int datafileCnt = START_FILE_NO; datafileCnt <= END_FILE_NO; ++datafileCnt)
	{
		...
		hp = new HashPipe(TOT_MEM_IN_BYTES);

		int packet_cnt = (int)traces[datafileCnt - 1].size();
		for(int i = 0; i < packet_cnt; ++i)
		{
			hp->insert((uint8_t*)(traces[datafileCnt - 1][i].key));
...
#define HEAVY_HITTER_THRESHOLD(total_packet) (total_packet * 1 / 1000)
		...
		hp->get_heavy_hitters(HEAVY_HITTER_THRESHOLD(packet_cnt), heavy_hitters);
